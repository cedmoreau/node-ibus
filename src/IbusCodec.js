...
export const commandTable = {
    ...
    [commands.GTMonitorCtrl]: {
        ...
        encode: ({ src, dst, power, source, encoding, aspect }) => {
            ...
            return { src, dst, msg: Buffer.from([commands.GTMonitorCtrl, data1, data2])};
        },
        decode: (data) => {
    ...
    [commands.BMBTB0]: {
        ...
        encode: ({ src, dst, released, hold, id }) => {
            const data1 = ((release << 7) & 0x80) | ((hold << 6) & 0x40) | (id & 0x3f);
    ...
    [commands.ANZVUpdate]: {
        encode: ({ src, dst, sub, data }) => {
            ...
            const subEntry = commandTable.ANZV_Update.subCommands[sub];
            if (!subEntry) throw new Error(`Unknown ANZV sub-command: ${sub}`);
            const msg = subEntry.encode(data);
            return { src, dst, msg: Buffer.concat([Buffer.from([commands.ANZV_Update, sub]), msg]) };
        },
        decode: (data) => {
